describe 'StripePayment' do
  ...
  describe '#perform_payment' do
    it 'on success' do
      ...
      expect(StripePayment.perform_transaction(params, cart).class).to eq Stripe::Charge
    end

    it 'on error' do
      allow(cart).to receive(:total).and_return 0
      expect(StripePayment.perform_transaction(params, cart)).to eq 'Invalid positive integer Please try again'
    end
  end
